...
def test_listUsers(url, payload, headers):
    response = requests.request("POST", url['listUsers'], data=json.dumps(payload), headers=headers)
    if response.status_code != 200:
        print(response.json())
    assert response.status_code == 200


def test_listUsers_no_token (url, payload, headers, monkeypatch):
...
#     response = requests.request("POST", url['listUsers'], data=payload, headers=headers)
#     # Test first login
