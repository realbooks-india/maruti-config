...
def test_listSegmap(url, payload, headers, monkeypatch):
    response = requests.request("POST", url['listSegmap'], data=json.dumps(payload), headers=headers)
    if response.status_code != 200:
        print(response.json())
    assert response.status_code == 200


# def test_listSegmap_no_token (url, payload, headers, monkeypatch):
...
#     response = requests.request("POST", url['listSegmap'], data=payload, headers=headers)
#     # Test first login
